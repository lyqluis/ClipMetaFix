...
- Delete-after-success: manual button + `AlertDialog`; R+ `createDeleteRequest`, Q29 `RecoverableSecurityException` sender, direct below; Document URIs via `deleteDocument` (attempted even on lookup miss); picker URIs resolved like any other via size/name lookup, manual only on miss. Safety rule: `overwrite` → delete A only (B IS the result); `insert` → delete A + old B.
